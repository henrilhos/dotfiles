...
typing_speed_cahce="$(mktemp -p '' typing_speed_cahce.XXXXX)"
trap 'rm "$typing_speed_cahce"' EXIT

# Write a dot to our cache for each key press
printf '' >"$typing_speed_cahce"
xinput test "$KEYBOARD_ID" |
    stdbuf -o0 awk '$1 == "key" && $2 == "press" && ('"$CONDITION"') {printf "."}' >>"$typing_speed_cahce" &
...
while true; do
    ...
    # cleared.
    lines=$(stat --format %s "$typing_speed_cahce")
