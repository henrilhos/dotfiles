...
Nixpkgs owns most of the CLI. Homebrew keeps the GUI casks plus two groups of CLI tools.

**Three nixpkgs can't provide on `aarch64-darwin`:**
...
**Seven where nixpkgs is simply worse** — each is both bigger and staler. Sizes are the nixpkgs
closure, versions are nixpkgs → brew at the time of the move:

| Item | nixpkgs closure | Version |
| --- | --- | --- |
| `gnumeric` | 1593 MB (whole GTK stack) | 1.12.60 → 1.12.61 |
| `markdownlint-cli` | 1556 MB (node closure, for a linter) | 0.48.0 → 0.49.1 |
| `cocoapods` | 863 MB (ruby closure) | 1.16.2 → 1.17.0 |
| `tmuxinator` | 804 MB (ruby closure) | 3.3.7 → 3.4.1 |
| `yazi` | 384 MB | 26.5.6 → 26.9.1 |
| `claude-code` | 344 MB, ships near-daily | 2.1.223 → 2.1.236 |
| `rtk` | 159 MB | 0.41.0 → 0.47.0 |

Together they deduplicate to 2.3 GB; dropping them took the user profile from 3.9 GB to 3.1 GB.

In every case only the *binary* moves — the configuration stays declarative here. Ghostty's config
is `modules/home/ghostty.nix`, tmuxinator's session files are still generated by
`modules/home/tmuxinator.nix` and read by the Homebrew binary.

Worth re-measuring when something feels heavy:

```sh
nix path-info -S "$(nix build --no-link --print-out-paths \
  '.#darwinConfigurations.tardis.config.home-manager.users.henrilhos.home.activationPackage')/home-path"
```
